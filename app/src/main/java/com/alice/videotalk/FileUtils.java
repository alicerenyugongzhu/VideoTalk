...
public class FileUtils {
    public static String getFileAbsolutePath(Activity context, Uri fileUri) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT && DocumentsContract.isDocumentUri(context, fileUri)) {
            Log.d("alice_debug", "SDK_INT larger than KITKAT");
            if (isExternalStorageDocument(fileUri)) {
                Log.d("alice_debug", "is external storage");
                ...
                Log.d("alice_debug", "is download document");
                String id = DocumentsContract.getDocumentId(fileUri);
                Uri contentUri = ContentUris.withAppendedId(Uri.parse("content://downloads/public_downloads"), Long.valueOf(id));
                return getDataColumn(context, contentUri, null, null);
            } else if (isMediaDocument(fileUri)) {
                Log.d("alice_debug", "is media document");
                String docId = DocumentsContract.getDocumentId(fileUri);
                String[] split = docId.split(":");
                String type = split[0];
                Uri contentUri = null;
            ...
            Log.d("alice_debug", "getScheme is content");
            // Return the remote address
            if (isGooglePhotosUri(fileUri)) {
                Log.d("alice_debug", "is google photo uri");
    ...
    public static String getDataColumn(Context context, Uri uri, String selection, String[] selectionArgs) {
        Log.d("alice_debug", "I am in the getDataColumn");
        Cursor cursor = null;
